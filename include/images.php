...
&& $_GET['pwg_token'] === get_pwg_token()
    ...
    // Initial request – build processing queue and store in session
    ...
    while ($index < count($queue) && $blockGenerated < $steps) {
        ...
        if ($overwriteThumbs || !$deriv->is_cached()) {
            ...
            if (!$simulate) {
                ...
                $rawUrl = ltrim($deriv->get_url(), '/');
                
                if (strpos($rawUrl, '?') !== false) {
                    [$script, $query] = explode('?', $rawUrl, 2);
                    } else {
                    $script = $rawUrl;
                    $query = '';
                }
                
                $script = trim($script); // e.g., "i.php"
                $query  = trim($query);  // e.g., "/galleries/..."
                
                // Encode the query part in order to process filenames with e.g. blanks correctly
                $encodedPath = implode('/', array_map('rawurlencode', explode('/', $query)));
